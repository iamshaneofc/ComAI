...
Tests:
    1. Create a Store
    2. Seed Products (bulk insert)
    3. Test General Product Search
    4. Test AI Chat (with real OpenAI call if key is set, or mock otherwise)
"""
import asyncio
import os
...
async def main():
    ...
    async with httpx.AsyncClient(base_url=API_URL, timeout=30.0) as client:
        ...
        # 1. Create a Store
        # ----------------------------------------------------------------
        console.print("\n[bold cyan]1. Creating Store...[/bold cyan]")
        store_res = await client.post("/stores", json={
        ...
        prod_res = await client.post("/products/bulk", json=products_payload, headers=headers)
        ...
        search_res = await client.get("/products/search", params={
            "keyword": "shoes",
            "max_price": 3000
        }, headers=headers)
